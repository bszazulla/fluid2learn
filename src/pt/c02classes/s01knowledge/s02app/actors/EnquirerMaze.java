...
public class EnquirerMaze implements IEnquirer {
	...
	public boolean discover() {
		...
		do {
			resposta = responder.ask(atual.getDirecao()); // pergunta ao
															// responder
			...
			if (resposta.equalsIgnoreCase("passagem")
				...
				if (atual.voltando()) {
					...
					if (!pilha.empty())
						dirUltimo = pilha.peek().getDirecao(); // ultima dire��o
																// escolhida
					else
						dirUltimo = "null"; // estah na entrada do labirinto e
											// pilha vazia

					atual = new DirecaoMaze(dirUltimo); // novo objeto apontado
														// por atual = nova
														// tentativa de
														// movimento
				}
